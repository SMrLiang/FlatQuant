...
def cali_flat_quant_internlm(args, model, dataloader, dev, logger):
    model.eval()
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    num_train_layer = len(layers)
    mse_dict = {}
    export_hidden_layers_prefill = False
    export_hidden_layers_decode = False
    for i in range(num_train_layer):
        logger.info(f"========= Layer {i} =========")
        dtype_dict = {}
        layer = layers[i].to(dev)
